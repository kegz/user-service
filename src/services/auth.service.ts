...
export const register = async (email: string, password: string, roleId: string, tenantId?: string) => {
...
export const login = async (email: string, password: string) => {
    ...
    const accessToken = jwt.sign({ sub: user._id, role: (user as any).role.name, tenant: user.tenant?.toString() }, config.jwtSecret, { expiresIn: "1h" });
    const refreshToken = crypto.randomUUID();
    const expiresAt = new Date(Date.now() + 7*24*60*60*1000);
    await RefreshTokenModel.create({ user: user._id, token: refreshToken, expiresAt });

    await UserModel.findByIdAndUpdate(user._id, { lastLogin: new Date() });
    return { accessToken, refreshToken, user: { id: user._id, email: user.email, role: (user as any).role.name } };
};
